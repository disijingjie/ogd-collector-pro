"""
4E-TOPSIS 综合分析脚本 v2
基于修复后的 table_platforms_fixed_22.csv 重新计算
...
INPUT_CSV = Path('data/verified_dataset/table_platforms_fixed_22.csv')
...
def load_fixed_data():
    ...
    print(f"  加载平台数: {len(df)}")
    
    # 确保数值类型
    numeric_cols = ['dataset_count', 'score_c1', 'score_c2', 'score_c3', 'score_c4', 
                    'app_count', 'has_api', 'has_bulk_download', 'has_https', 'has_search',
                    'has_download', 'has_visualization', 'has_update_info', 'has_metadata',
                    'has_feedback', 'has_register', 'has_preview']
    for col in numeric_cols:
        df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0)
    
    # 添加province字段（与name相同）
    df['province'] = df['name']
    
    return df

def compute_4e_indicators(df):
...
def export_topsis(df):
    ...
    print("\n=== 4E-TOPSIS排名（修复后） ===")
    print(f"{'排名':<4} {'平台':<14} {'得分':<8} {'梯队':<8} {'E1供给':<8} {'E2服务':<8} {'E3质量':<8} {'E4利用':<8} {'E5公平':<8}")
    print("-" * 85)
    for _, row in df_sorted.iterrows():
        print(f"{row['topsis_rank']:<4} {row['name']:<14} {row['topsis_score']:<8.4f} "
...
def compute_dea(df):
    """DEA-BCC效率分析（纯NumPy实现）
    投入：运营年限、功能得分、数据集数量
    产出：TOPSIS得分
    """
    df_dea = df.copy()
    # 使用2024作为参考年份计算运营年限
    df_dea['operating_years'] = 2024 - df_dea.get('launch_year', 2015)
    df_dea['operating_years'] = df_dea['operating_years'].clip(lower=1)
    
    # 投入指标
    inputs = np.array([
        df_dea['operating_years'].values,
        df_dea['E2_func'].values * 11,
        np.log1p(df_dea['dataset_count'].values)
    ]).T
    
    # 产出指标
    outputs = np.array([df_dea['topsis_score'].values]).T
    
    n = len(df_dea)
    efficiencies = []
    
    for i in range(n):
        weighted_output = outputs[i, 0]
    ...
    print("\n=== DEA-BCC效率排名 ===")
    print(f"{'排名':<4} {'平台':<14} {'效率':<8} {'运营年':<6} {'功能':<6} {'数据集':<8} {'TOPSIS':<8}")
    print("-" * 65)
    for _, row in df_dea.sort_values('dea_efficiency', ascending=False).iterrows():
        print(f"{row['dea_rank']:<4} {row['name']:<14} {row['dea_efficiency']:<8.4f} "
...
def compare_with_before(df_new):
    """对比修复前后的排名变化"""
    # 读取之前的TOPSIS结果
    old_files = sorted(OUTPUT_DIR.glob('table_topsis_4e_20260426_*.csv'))
    if not old_files:
        print("\n  未找到之前的TOPSIS结果，跳过对比")
        return
    ...
    print("\n=== 修复前后排名对比 ===")
    print(f"{'平台':<14} {'旧排名':<6} {'新排名':<6} {'变化':<6} {'旧得分':<8} {'新得分':<8} {'得分变化':<8}")
    print("-" * 70)
    ...
    for _, row in df_new.iterrows():
        ...
        if old_rank != '-':
            ...
            print(f"{row['name']:<14} {old_rank:<6} {new_rank:<6} {change_str:<6} {old_score:<8.4f} {new_score:<8.4f} {score_change:+.4f}")
        else:
            print(f"{row['name']:<14} {'新':<6} {new_rank:<6} {'-':<6} {'-':<8} {new_score:<8.4f} {'-':<8}")

def main():
    print("=" * 80)
    print("4E-TOPSIS 综合分析 v2（基于修复后数据）")
    print("=" * 80)
